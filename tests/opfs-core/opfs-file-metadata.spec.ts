...
test.describe('OPFS File Listing', () => {
  test('File Metadata Display', async ({ page }) => {
    ...
    await editorName.fill('metadata.txt');
    const editorTextarea = page.locator('textarea');
    await editorTextarea.fill('Hello metadata test');
    await page.locator('text=Create or update a file, button').click();
    await expect(page.locator('text=Uploaded "metadata.txt"')).toBeVisible();

    // Step 2: Verify the file listing displays the correct file size
    const listing = page.locator('.listing, .directory-list, table');
    await expect(listing).toContainText('metadata.txt');

    // Step 3: Verify the file listing displays the correct last modified date
    await expect(page.locator('text=metadata.txt')).toBeVisible();

    // Step 4: Verify the file listing shows "file" as the type for regular files
    const fileRow = listing.locator('text=metadata.txt');
    await expect(fileRow).toBeVisible();

    // Step 5: Verify directories show "(N/A)" for size and modified date
    const dirNameInput = page.locator('input[placeholder*="dir"], input[name*="dir"]');
    await dirNameInput.click();
    await dirNameInput.fill('metadir');
    await page.locator('text=Create new directory, button').click();

    const dirRow = page.locator('text=metadir/');
    await expect(dirRow).toBeVisible();

    // Step 6: Verify directory names end with "/" in the listing
    await expect(dirRow).toContainText('metadir/');
  });
  ...
  test('File Handle Locked by DuckDB', async ({ page, browser }) => {
    ...
    // The editor should show an alert about the file being locked
    await editorName.fill('locked.csv');
    await page.locator('text=Create or update a file, button').click();
