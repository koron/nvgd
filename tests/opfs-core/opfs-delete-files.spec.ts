...
test.describe('File Deletion Operations', () => {
  test('Delete Selected Files', async ({ page }) => {
    ...
    await page.locator('textarea').fill('Content of file 2');
    await page.locator('text=Create or update a file, button').click();
    await expect(page.locator('text=Uploaded "file2.txt"')).toBeVisible();

    // Step 2: Check the checkbox next to `file1.txt`
    const file1Checkbox = page.locator('text=file1.txt').locator('input[type=checkbox], [class*="checkbox"], [class*="check"]');
    await file1Checkbox.check();

    // Step 3: Check the checkbox next to `file2.txt`
    const file2Checkbox = page.locator('text=file2.txt').locator('input[type=checkbox], [class*="checkbox"], [class*="check"]');
    await file2Checkbox.check();

    // Step 4: Verify the "Delete" button becomes enabled
    ...
    // Step 6: Verify a confirmation dialog appears listing the files to delete
    const confirmDialog = page.locator('text=Confirm, text=delete');
    await expect(confirmDialog).toBeVisible();

    // Step 7: Confirm the deletion
    await page.locator('text=Confirm, button, :text("Confirm")').click();

    // Step 8: Verify both files are removed from the directory listing
    await expect(page.locator('text=file1.txt')).not.toBeVisible();
    await expect(page.locator('text=file2.txt')).not.toBeVisible();

    // Step 9: Verify the "Delete" button becomes disabled again
    await expect(deleteBtn).toBeDisabled();

    // Step 10: Delete a file with an empty name
    await page.locator('text=Delete, button').click();
    const emptyDeleteAlert = page.locator('text=Need selection, text=Error');
    await expect(emptyDeleteAlert).toBeVisible();
  });
  ...
  test('Delete Single File', async ({ page }) => {
    ...
    // (Only single.txt was created, so this verifies no other files appeared)
