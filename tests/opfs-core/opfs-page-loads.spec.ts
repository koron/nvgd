...
test.describe('OPFS Page Loading and Initial State', () => {
  test('OPFS Page Loads and Displays Root Directory Listing', async ({ page }) => {
    await page.goto('http://localhost:9280/opfs/');

    // Verify the page title is OPFS: (Root)
    await expect(page).toHaveTitle(/OPFS: \(Root\)/);

    // Verify the page loads successfully with no JavaScript errors
    const consoleErrors = [];
    page.on('console', msg => {
    ...
    // Verify the breadcrumb shows (Root)
    const breadcrumb = page.locator('text=(Root)');
    await expect(breadcrumb).toBeVisible();

    // Verify the directory listing displays the root contents
    const listing = page.locator('.listing, .directory-list, table, .file-list');
