...
test.describe('Browser Compatibility Considerations', () => {
  test('OPFS Operations in Secure Context', async ({ page }) => {
    // Verify the OPFS page loads in a secure context (localhost)
    await page.goto('http://localhost:9280/opfs/');

    // Verify all OPFS operations work correctly in a secure context
    // Verify file upload, download, and editor operations work
    const editorName = page.locator('input[name*="file"], input[placeholder*="file"], input[type=text]');
    await editorName.fill('secure-test.txt');
    const editorTextarea = page.locator('textarea');
    await editorTextarea.fill('Secure context test');
    await page.locator('text=Create or update a file, button').click();
    await expect(page.locator('text=Uploaded "secure-test.txt"')).toBeVisible();

    // Verify DuckDB integration works in a secure context
    const csvCheckbox = page.locator('text=secure-test.txt').locator('input[type=checkbox], [class*="checkbox"], [class*="check"]');
